...
{ 
   ...
   */
  int rem_bt[n];    //number 1
  for (int x = 0; x < n; x++) 
  {
      rem_bt[x] = plist[x].bt;  //Number 2
  }
...
{ 
      ...
      * 1 Traverse until all process gets completely executed.
        ...
        f Calculate waiting time for each completed process. *wt[i]= Completion time - arrival_time-burst_time*
        g Increment time lap by one.

    */

  int rt[n]; 
  int complete = 0, t = 0, shortest = 0, is_done = 0;
  int minm = INT_MAX, finish_time;
  for (int i = 0; i < n; i++) 
  {
    rt[i] = plist[i].bt; 
  }
...
{ 
  ...
  // else if priorities equal,sort by arrival time
  if ((p1->pri - p2->pri) == 0) 
  {
